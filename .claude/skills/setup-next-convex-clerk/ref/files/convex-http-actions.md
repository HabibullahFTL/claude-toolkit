...
Apply sections in order. "Always" sections run unconditionally. Conditional sections run only
when the matching flag is true. Apply **[If wants_file_storage=true]** before **[If wants_clerk=true]**
so the `index.ts` merge is always additive.
...
export const clerkHttpActions = {
  webhook: httpAction(async (ctx, request) => {
    ...
    if (event.type === 'user.deleted') {
      ...
      if (data.id) {
        await ctx.runMutation(
          internal.functions.users.internal.deleteUserByClerkId,
          { clerkId: data.id },
        );
      }
    }
...
Read the existing file. Add `internalMutation` to the import from `../../_generated/server` if not already present. Append both mutations below `userByClerkIdInternal`:
  ...
  handler: async (ctx, { clerkId, email, name }) => {
    ...
    if (existing) {
      ...
      await ctx.db.insert('users', {
        ...
        email,
        ...(name ? { name } : {}),
        // ADAPT: add required schema fields here (see SKILL.md Step 5.9 adaptation notes)
      });
    }
  },
});

// [INTERNAL-FN]: Called when Clerk fires user.deleted.
// Deletion is intentionally not implemented — add your own logic here if needed.
// Options: hard delete (ctx.db.delete), soft delete (patch status/isDeleting),
// or cascade-delete related documents before removing the user row.
export const deleteUserByClerkId = internalMutation({
  args: {
    clerkId: v.string(),
  },
  handler: async (_ctx, { clerkId }) => {
    console.log(`user.deleted received for clerkId: ${clerkId} — no action taken`);
  },
});
```
...
2. Replace the empty object (merge with any existing entries from the file storage step):
   ```ts
   // before
   export const httpActionsHandlers = {};
   // after
   export const httpActionsHandlers = {
     // ...existing entries (e.g. convexStorage) if wants_file_storage=true...
...
4. If cross-origin POST/PATCH/DELETE, add the path to `PREFLIGHT_PATHS` and register the OPTIONS handler
